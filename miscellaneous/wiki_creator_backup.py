...
class WikiCreator(object):
    ...
    def format_page(self, flare):
        ...
        for instr in sections:
            ...
            for ins in instr[2]:
                fields.append(f'<img style="max-height:350px;" src="{HOST}/request/image/flare?id={flare_id}&type={ins}&uid={uid}&p=0"></img>')
        utc_dt=dtparser.parse(peak_utc)
        ymd=utc_dt.strftime('%Y%m%d')
        fields.append(f'\n== Radio ==\n')
        fields.append(f'<img  style="max-height:350px;" src="http://ovsa.njit.edu/flaremon/daily/{utc_dt.year}/XSP{ymd}.png"></img>')
        if fields:
            content='\n'.join(fields)+'\n'
