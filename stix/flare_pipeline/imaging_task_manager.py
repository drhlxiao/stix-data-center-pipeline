...
def queue_imaging_tasks(doc,
    ...
    try:
        ...
        logger.warning(
            f'No background data found for BSD {bsd_id} (uid {uid})')
        return
    ...
    bsd_flare_time_ranges = [{
        'start':
        max(x['start_unix'], bsd_start_unix),
        'peak':
        x['peak_unix_time']
        if bsd_start_unix <= x['peak_unix_time'] <= bsd_end_unix else None,
        'end':
        min(x['end_unix'], bsd_end_unix)
    } for x in mdb.find_flares_by_time_range(bsd_start_unix, bsd_end_unix)]

    boxes = l1.get_time_ranges_for_imaging(bsd_flare_time_ranges, energy_bands,
                                           min_counts, duration)
