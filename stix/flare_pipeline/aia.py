...
def get_AIA_observer(date_obs):
    ...
    try:
        ...
        positions = eph.get_earth_spice_HEE(
            [pd.to_datetime(stix_bp_map.meta['date_obs'])])['positions'][0]
...
def find_cutout_coords(stix_bp_map, padding=10 * u.arcsec):
    ...
    sdo_refcoord = get_AIA_observer(
        pd.to_datetime(
            stix_bp_map.meta['date-obs']))  #warning no light-time correction
...
def download_AIA_cutout(time_int,
                        wave=1600,
                        series='aia_lev1_uv_24s',
                        cutout_coords=False,
                        single_result=True,
                        aia_file_path=None):
    ...
    if not aia_file_path:
        files = Fido.fetch(
            res,
            path='./')  #set path in config - destination for downloaded files
    else:
