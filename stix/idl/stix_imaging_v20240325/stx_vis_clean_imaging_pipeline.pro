...
function stx_vis_clean_imaging_pipeline, vis, aux_data, niter = niter, image_dim = image_dim_in, pixel = pixel, $
  _extra = _extra,  $
  ...
  ; CLEAN MAP
  out0 = stx_make_map_imaging_pipeline(clean_image, aux_data, [pixel,pixel], "CLEAN", vis)
  out1 = stx_make_map_imaging_pipeline(bproj_map, aux_data, [pixel,pixel], "BPROJ", vis)
  out2 = stx_make_map_imaging_pipeline(resid_map, aux_data, [pixel,pixel], "CLEAN RESIDUAL", vis)
  out3 = stx_make_map_imaging_pipeline(clean_sources_map, aux_data, [pixel,pixel], "CLEAN COMPONENTS", vis)
  out4 = stx_make_map_imaging_pipeline(clean_map, aux_data, [pixel,pixel], "CLEAN CONVOLVED", vis)

  resid_map         = out2.solo_hpc_ref_map.data
  clean_sources_map = out3.solo_hpc_ref_map.data
  clean_map         = out4.solo_hpc_ref_map.data
  ...
  if keyword_set(plot) then begin
    ...
    !p.multi=[0,3,1]
    chs2=2.
    plot_map,out0.solo_hpc_ref_map,charsize=chs2
    this_dmax=max(out1.solo_hpc_ref_map.data)
    this_dmin=min(out1.solo_hpc_ref_map.data)
    plot_map,out1.solo_hpc_ref_map,charsize=chs2,dmin=this_dmin,dmax=this_dmax
    plot_map,out2.solo_hpc_ref_map,/per,/over,level=[50]
    plot_map,out2.solo_hpc_ref_map,charsize=chs2,dmin=this_dmin,dmax=this_dmax
    plot_map,out2.solo_hpc_ref_map,/per,/over,level=[50]

    peak_residual=strmid(strtrim(max(out2.solo_hpc_ref_map.data)/max(out1.solo_hpc_ref_map.data)*100.,2),0,4)+'%'
    xyouts,0.065,0.23,'peak residual at '+peak_residual,/normal,charsize=2
  endif


  out_stx_ref     =[out0.stx_ref_map,out1.stx_ref_map,out2.stx_ref_map,out3.stx_ref_map,out4.stx_ref_map]
  out_solo_hpc_ref=[out0.solo_hpc_ref_map,out1.solo_hpc_ref_map,out2.solo_hpc_ref_map,out3.solo_hpc_ref_map,out4.solo_hpc_ref_map]
  
  out = {stx_ref_map: out_stx_ref,$
        solo_hpc_ref_map: out_solo_hpc_ref}
