...
pro stx_estimate_flare_location_imaging_pipeline, path_sci_file, time_range, aux_data, flare_loc=flare_loc, path_bkg_file=path_bkg_file, $
  energy_range=energy_range, imsize=imsize, mapcenter=mapcenter, $
  subc_index=subc_index, silent=silent, this_win=this_win, sidelobes_ratio=sidelobes_ratio, $
  bp_map_full_disk=bp_nat_map, _extra=extra
  ...
  bp_nat_map = stx_bproj_imaging_pipeline(vis,imsize,pixel,aux_data)
  bp_nat_map = bp_nat_map.solo_hpc_ref_map
  ...
  vis_fwdfit_pso_map = stx_vis_fwdfit_pso_imaging_pipeline(configuration,vis,aux_data,srcstr=srcstr,/silent)
  vis_fwdfit_pso_map = vis_fwdfit_pso_map.solo_hpc_ref_map
