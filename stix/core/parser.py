...
from stix.core import header as stxhd
from stix.core import idb
from stix.core import const
from stix.core import logger
from stix.core import io
...
def find_next_header(buf, i):
    ...
    while i < length:
        x = buf[i]
        if x in stxhd.HEADER_FIRST_BYTE:
            return i
        else:
            i += 1
    return const.EOF
...
class StixVariableTelemetryPacketParser(StixParameterParser):
    ...
    def parse(self, data, spid):
        ...
        if self.length_min > packet_length:
            return 0, None, const.VARIABLE_PACKET_LENGTH_MISMATCH
        self.walk(self.nodes[0], self.results_tree)
        return self.current_offset, self.results_tree, const.OK
    ...
    def build_parse_tree(self):
        ...
        repeater = [{
            'node': mother,
            'counter': const.MAX_NUM_PARAMETERS
        }]
...
class StixContextParser(StixParameterParser):
    ...
    def parse(self, buf):
        ...
        parameters = []
        param_id = 0
        for name, width in context.CONTEXT_PARAMETER_BIT_SIZE:
            #width also in units of bits
            offset_bytes = int(offset / 8)
            offset_bits = offset % 8
            children = []
            raw_values = None
            if name in context.ASIC_REGISTERS:
    ...
    def parse_asic_registers(self, buf, offset):
        parameters = []
        for name, width in context.CONTEXT_REGISTER_BIT_SIZE:
            offset_bytes = int(offset / 8)
            offset_bits = offset % 8
            ...
            if raw_value is not None and raw_value != '':
                param = (context.CONTEXT_REGISTER_DESC[name], raw_value,
                         '', [])
                parameters.append(param)
        return parameters
...
class StixTelecommandParser(StixParameterParser):
    ...
    def parse_fixed_telecommand(self):
        ...
        return int(self.current_bit_offset / 8), params, const.OK
...
class StixTCTMParser(StixParameterParser):
    ...
    def parse_telemetry_header(self, packet):
        ...
        if len(packet) < 16:
            ...
            return const.PACKET_TOO_SHORT, None
        if ord(packet[0:1]) not in stxhd.TM_HEADER_FIRST_BYTE:
            return const.HEADER_FIRST_BYTE_INVALID, None
        header_raw = st.unpack('>HHHBBBBIH', packet[0:16])
        header = {}
        for h, s in zip(header_raw, stxhd.TELEMETRY_RAW_STRUCTURE):
            header.update(unpack_integer(h, s))
        status = self.check_header(header, 'tm')
        if status == const.OK:
            header['segmentation'] = stxhd.PACKET_SEG[header['seg_flag']]
            header['TMTC'] = 'TM'
            header[
    ...
    def check_header(self, header, tmtc='tm'):
        ...
        constrains = None
        if tmtc == 'tm':
            constrains = stxhd.TELEMETRY_HEADER_CONSTRAINTS
        else:
            constrains = stxhd.TELECOMMAND_HEADER_CONSTRAINTS
        for name, lim in constrains.items():
            if header[name] not in lim:
                logger.warning(
                    'Header {} value {} violates the range: {} '.format(
                        name, header[name], lim))
                return const.HEADER_INVALID
        return const.OK
    ...
    def parse_telecommand_header(self, buf, ipos):
        ...
        if buf[ipos] not in stxhd.TC_HEADER_FIRST_BYTE:
            return const.HEADER_FIRST_BYTE_INVALID, None
        try:
        ...
        if not info:
            ...
            return const.HEADER_KEY_ERROR, header
    ...
    def parse_binary(self, buf, i=0, header_auxiliary=None):
        ...
        while i < length and not self.stop_parsing:
            ...
            elif buf[i] in stxhd.TC_HEADER_FIRST_BYTE:
            ...
            if packet:
                ...
                if self.packet_io:
                    self.packet_io.write_one(packet)
    ...
    def attach_timestamps(self, packet):
        ...
        if pkt_header['TMTC'] == 'TM':
            ...
            pkt_header['obt_utc'] = sdt.scet2utc(coarse, fine)
            if not use_receipt_time:
                pkt_header['unix_time'] = sdt.scet2unix(coarse, fine)
                pkt_header['UTC'] = sdt.unix2utc(
        ...
        with open(filename) as f:
            ...
            for item in doc['ns2:ResponsePart']['Response'][
                ...
                header_auxiliary = {
                    ...
                    'unix_time': sdt.utc2unix(item['ExecutionTime']),
    ...
    def parse_file(self, raw_filename, file_type=None, clear=True):
        ...
        if self.packet_io:
            self.packet_io.set_filename(raw_filename)
    ...
    def is_processed(self, filename):
        return self.packet_io.is_processed(filename)
