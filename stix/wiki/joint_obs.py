...
def process_flares_for_file(file_id, overwrite=True):
    ...
    for doc in flares:
        ...
        aia.plot_aia(folder,_id, flare_id, peak_utc,  wavelen=131, overwrite=overwrite)
        print('processing goes')
        goes.plot_goes(folder,_id, flare_id, start_utc, end_utc, overwrite=overwrite)
        try:
