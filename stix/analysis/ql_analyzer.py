...
def interp_counts(lc_times, lc_counts, tstep=4):
    """
    background QL has an integration time of 8 sec while QL 4 sec
    """
    f = interpolate.interp1d(lc_times, lc_counts, fill_value="extrapolate")
    tx = np.arange(lc_times[0], lc_times[-1] + tstep, tstep)
...
class QLAnalyzer(object):
    ...
    def peek(self, ax=None, ilcs=[0, 1, 2, 3, 4]):
        ...
        for i in ilcs:
            ax.plot(ql_times,
                    data['lcs'][i],
                    label=f'QL LC {data["energy_bins"]["names"][i]}')
        if 'rcr' in data:
...
class LightCurveAnalyzer(QLAnalyzer):
    ...
    def from_database(cls, start_utc, end_utc):
        start_unix, end_unix = time_utils.anytime(start_utc,
                                                  'unix'), time_utils.anytime(
                                                      end_utc, 'unix')
        packets = mdb.get_quicklook_packets('lc',
                                            start_unix,
                                            end_unix - start_unix,
                                            sort_field='_id')
    ...
    def parse(packets,
        ...
        for pkt in packets:
            ...
            if not energy_bins:
                ...
                energy_bins = StixEnergyBins.get_emask_energy_bins(
                    energy_bin_mask, tstart)
        ...
        return {
            'time': utime,
            't': utime - utime[0],
            'lcs': {x: np.array(lightcurves[x])[sel]
...
class BKGLigthCurveAnalyzer(QLAnalyzer):
    ...
    def from_database(cls, start_utc, end_utc):
        start_unix, end_unix = time_utils.anytime(start_utc,
                                                  'unix'), time_utils.anytime(
                                                      end_utc, 'unix')
        packets = mdb.get_quicklook_packets('bkg',
                                            start_unix,
                                            end_unix - start_unix,
                                            sort_field='_id')
...
class LightCurveMerger(QLAnalyzer):
    ...
    def __init__(self, qllc={}, qlbkg={}):
        ...
        self._data = {'BKG_LC': qlbkg.data, 'QL_LC': qllc.data}
    ...
    def peek(
        ...
        for i in ilcs:
            ax.plot(
                bkg_dt,
                data['BKG_LC']['lcs'][i],
                label=f'BKG LC {data["BKG_LC"]["energy_bins"]["names"][i]}')
            ax.plot(ql_dt,
                    data['QL_LC']['lcs'][i],
                    label=f'QL LC {data["QL_LC"]["energy_bins"]["names"][i]}')
    ...
    def correct_att_in_counts(self, tgap=20):
        ...
        ql_before_in = ql_lcs[:, ql_time_sel]
        ql_before_in=ql_before_in.flatten().reshape(5,-1)
        #squeeze, remove unneeded dimemsion
        ...
        bkg_after_in = bkg_lcs[:, bkg_time_sel]
        bkg_after_in=bkg_after_in.flatten().reshape(5,-1)
        ...
        for i in range(5):
            ...
            if bkg_min_cnts[i] <= BKG_baseline[i] + 3 * np.sqrt(
                    BKG_baseline[i]):
                # too noisy, not valid
                slopes.append(0)
            else:
        ...
        bkg_att_in_lcs = []
        for i in range(5):
            bkg_att_in_lcs.append(
                np.array(bkg_lcs[i][sel_t_bkg] * slopes[i] + QL_baseline[i]
                         ) if slopes[i] > 0 else bkg_lcs[0][sel_t_bkg] * 0)

        bkg_att_in_times = bkg_times[sel_t_bkg]

        time_4sec, counts_4sec = interp_counts(bkg_att_in_times,
                                               bkg_att_in_lcs,
                                               tstep=4)
        ...
        return {
            ...
            counts_4sec,
            'ql_time_before_in':
            [time_utils.unix2utc(x) for x in ql_time_before_in],
            'ql_max_cnts_before_in':
            ql_max_cnts,
            'bkg_min_cnts_after_in':
            bkg_min_cnts,
            'ql_bkg':
            QL_baseline,
            'ql_counts_before_in':
