...
SPECTRUM_MODEL_DATA_FILES = {
    'EXP': Path(__file__).parent.parent / 'data' / 'ExpSpec.dat',
...
class Calibration(object):
    ...
    def get_subspec(spectrum, start_ch, bin_width, only_fit_range=True):
        ...
        min_adc = peak_x - (31 - Calibration.FIT_RANGE[0]
                            ) * Calibration.MEAN_ENERGY_CONVERSION_FACTOR
        max_adc = peak_x + (-31 + Calibration.FIT_RANGE[1]
                            ) * Calibration.MEAN_ENERGY_CONVERSION_FACTOR
    ...
    @staticmethod
    def get_intensity_from_model(offset: float, slope: float,
    ...
    def correlate(offset, slope, spec_x: np.ndarray, spec_y: np.ndarray):
        ...
        Returns
         correlation factor
        """
        energies, pred_spec = Calibration.get_intensity_from_model(
            offset, slope, spec_x)
    ...
    @staticmethod
    def quick_random_search(offsets_1d,
                            slopes_1d,
                            spec_x,
                            spec_y,
                            max_delta=1e-3,
        ...
        offset_next_range = (np.max(offsets_1d) -
                             np.min(offsets_1d)) / Calibration.SCALE_FACTOR
        slope_next_range = (np.max(slopes_1d) -
                            np.min(slopes_1d)) / Calibration.SCALE_FACTOR
    ...
    def find_solution(detector,
        ...
        """
        spec_x, spec_y = Calibration.get_subspec(spectrum, start_ch,
        ...
        peak3_peak_range = np.array(
            [peak1_x + 0.9 * x_shift, peak1_x + 1.1 * x_shift])
        peak3_x_range = (spec_x > peak3_peak_range[0]) & (spec_x <
                                                          peak3_peak_range[1])
        ...
        best_fit = Calibration.quick_random_search(offsets_1d, slopes_1d,
                                                   spec_x, spec_y)

        # print(best_fit)
        energies, pred_spec = Calibration.get_intensity_from_model(
            best_fit['offset'], best_fit['slope'], spec_x)
    ...
    def get_sum_spectrum(spectra, slope: np.ndarray, offset: np.ndarray):
        ...
        # cc = TCanvas()
        pixel_sum_spectra = np.zeros(
            (32, 12, 1000))  # pixel sum spectrum, 100 energy bins
        ...
        for spec in spectra:
            ...
            if slope[detector][pixel] > 0 and offset[detector][pixel] > 0:
                ...
                spec_x, spec_y = Calibration.get_subspec(
                    spectrum, start, bin_width, False)
                energies = (spec_x -
                ...
                yvals = Calibration.interp(energies, spectrum / bin_width,
                                           xvals)
                sum_spectra[sbspec_id][1] += yvals
                amps = Calibration.interp(energies, spectrum / bin_width,
                                          pixel_ebins)
        ...
        for key, val in sum_spectra.items():  # mongodb doesn't support array
            ...
            sbspec_sum += Calibration.interp(sum_spectra[key][0],
                                             sum_spectra[key][1], energy_range)
        ...
        np.save(
            os.path.join(Calibration.DEFAULT_OUTPUT_DIR,
                         'pixel_sum_spectra.npy'), pixel_sum_spectra)
    ...
    def process_one_run(calibration_id, create_pdf=True):
        ...
        if create_pdf:
            pdf_filename = os.path.abspath(
                os.path.join(
                    ...
                    f'calibration_{calibration_id}_{Calibration.MINIMIZATION_METHOD}.pdf'
                ))
        ...
        for spec in spectra:
            ...
            if start > Calibration.SPECTRUM_SEL_ADC_RANGE[
                ...
                continue
            best_fit = Calibration.find_solution(detector, pixel, sbspec_id,
                                                 spectrum, start, bin_width,
                                                 pdf)
            ...
            slope_error[detector][pixel] = 0.5 * np.sqrt(2) * bin_width / (81 -
                                                                           30)
