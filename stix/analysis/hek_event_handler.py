...
from stix.analysis import rotate_coord
from stix.spice import joint_visible as jv
...
class HEKEventHandler():
    ...
    def get_observers_and_wcs(self):
        ''' Get observers and WCS for use in coordinate rotations'''
        self.observer_in,self.wcs_in=jv.get_observer(self.date_obs,obs=self.obs_instrument,hee=self.hee_in)
#        try:
...
#        except AttributeError:
        self.observer_out,self.wcs_out=jv.get_observer(self.date_obs,obs=self.obs_out,hee=self.hee_out)
    ...
    def rotate_hek_coords(self,df,binning=1):
        ...
        for i,row in df.iterrows():
            rc=rotate_coord.rotate_coord(row.hpc_x,row.hpc_y,self.observer_in,self.wcs_in,obs_out=self.observer_out,wcs_out=self.wcs_out,binning=binning) #.iloc[0]
            try:
                rc.do_rotation()
            except ValueError:
                print(f"Coordinate ({row.hpc_x},{row.hpc_y})could not be rotated!")
            ...
            if self.hee_out: #to do: get hee_in and hee_out from observers...
                rdf['visible_from_SOLO']=jv.is_visible_from_SO(self.date_obs, (row.hpc_x,row.hpc_y),obs_out=self.hee_out)
            else:
                rdf['visible_from_SOLO']=jv.is_visible_from_SO(self.date_obs, (row.hpc_x,row.hpc_y))
            dfs.append(rdf)
