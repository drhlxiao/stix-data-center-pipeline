...
class PeakFitting(object):
    ...
    def fit_double_gaus(x,
        ...
        moda = Model(PeakFitting.gaussian, prefix='a_') + Model(
            PeakFitting.gaussian, prefix='b_') + Model(PeakFitting.constant)
        pars = moda.make_params()
        print(a_cen, b_cen)
...
class Calibration(object):
    ...
    def get_subspec(self, spectrum, start_ch, bin_width, only_fit_range=True):
        ...
        if only_fit_range:
            clip = (_x > Calibration.FIT_RANGE[0]) & (_x <
                                                      Calibration.FIT_RANGE[1])
    ...
    def find_solution(self,
        ...
        peak1_2_mean_shift = (PHOTO_PEAKS_POS[1] - PHOTO_PEAKS_POS[0]
                              ) * Calibration.MEAN_ENERGY_CONVERSION_FACTOR
        peak1_3_mean_shift = (PHOTO_PEAKS_POS[2] - PHOTO_PEAKS_POS[0]
                              ) * Calibration.MEAN_ENERGY_CONVERSION_FACTOR

        peak1_x, peak1_y, peak1_pos = select_near_max(
            spec_x, spec_y, peak1_pos - Calibration.FIT_RANGE_SMALL_MARGIN,
            peak1_pos + Calibration.FIT_RANGE_SMALL_MARGIN,
            Calibration.FIT_RANGE_SMALL_MARGIN,
            Calibration.FIT_RANGE_SMALL_MARGIN)

        peak2_x, peak2_y, peak2_pos = select_near_max(
            spec_x, spec_y, peak1_pos + 0.5 * peak1_2_mean_shift,
            peak1_pos + 1.3 * peak1_2_mean_shift,
            Calibration.FIT_RANGE_SMALL_MARGIN,
            Calibration.FIT_RANGE_SMALL_MARGIN)

        peak12_x, peak12_y, peak12_pos = select_near_max(
            spec_x, spec_y, peak1_pos - Calibration.FIT_RANGE_SMALL_MARGIN,
            peak2_pos + Calibration.FIT_RANGE_BIG_MARGIN,
            Calibration.FIT_RANGE_SMALL_MARGIN,
            peak2_pos - peak1_pos + Calibration.FIT_RANGE_BIG_MARGIN)

        low_e_result = PeakFitting.fit_double_gaus(peak1_x, peak1_y, peak1_pos,
                                                   peak2_x, peak2_y, peak2_pos,
                                                   peak12_x, peak12_y)
        print(low_e_result)

        peak3_x, peak3_y, peak3_pos = select_near_max(
            spec_x, spec_y, peak1_pos + 0.85 * peak1_3_mean_shift,
            peak1_pos + 1.15 * peak1_3_mean_shift,
            Calibration.FIT_RANGE_SMALL_MARGIN,
            Calibration.FIT_RANGE_BIG_MARGIN)
        high_e_result = PeakFitting.fit_asym_gaus(peak3_x, peak3_y, bin_width)
        # print(high_e_result.fit_report())
        peak_centers = np.array([
            low_e_1_result.params['cen'].value,
            low_e_2_result.params['cen'].value,
            high_e_result.params['cen'].value
        ])
        ...
        peak_center_err = np.array(
            [0.25 * bin_width, 0.25 * bin_width, 0.25 * bin_width])
        # print('line')
        ...
        linfit_result = PeakFitting.fit_linear(PHOTO_PEAKS_POS, peak_centers,
                                               peak_center_err)
        ...
        try:
            ...
            print(
                f"ERROR: D {detector}-{pixel} slope, offset error not available."
            )
            slope_error = 0.5 * bin_width / (81 - 31)
            offset_error = 0.5 * bin_width
        best_fit = {
            'slope': round(linfit_result.params['slope'].value, 3),
        ...
        if pdf:
            ...
            plt.suptitle(
                f'Detector {detector},pixel {pixel}, subspec {sbspec_id} ')
            axs[0].legend()
            axs[1].errorbar(PHOTO_PEAKS_POS,
                            peak_centers,
                            yerr=peak_center_err,
                            fmt='o',
                            label='Obs photopeaks')
            axs[1].plot(PHOTO_PEAKS_POS,
                        linfit_result.best_fit,
                        'k',
                        label='Linear fit')
    ...
    def process_one_run(self,
        ...
        if create_pdf:
            pdf_filename = os.path.abspath(
                os.path.join(Calibration.DEFAULT_OUTPUT_DIR,
                             'calibration_{}_gaus.pdf'.format(calibration_id)))
        ...
        for spec in spectra:
            ...
            if start > Calibration.FIT_RANGE[0] or end < Calibration.FIT_RANGE[
                    1]:
                continue
            try:
                best_fit = self.find_solution(detector, pixel, sbspec_id,
                                              spectrum, start, bin_width, pdf)
            except:
                raise
                continue
