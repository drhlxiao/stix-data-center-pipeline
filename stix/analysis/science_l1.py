...
class ScienceData():
    ...
    def read_fits(self, light_time_correction=True):
        ...
        self.T0_utc = _get_from_keys(self.hdul['PRIMARY'].header,
                                     ['DATE_BEG', 'DATE-BEG'])
        ...
        self.energy_bin_mask = _get_from_keys(
            self.hdul["CONTROL"].data,
...
class ScienceL1(ScienceData):
    ...
    def get_time_and_counts(self,
        ...
        if start is None or end is None:
            ...
            peak_time_bin = [
                self.time_bins_low[max_idx], self.time_bins_high[max_idx]
            ]
            ...
            if peak_time_bin[1] - peak_time_bin[0] >= integration_time:
                ...
                start = max(peak_time - integration_time / 2,
                            self.time_bins_low[0])
                end = min(peak_time + integration_time / 2,
                          self.time_bins_high[-1])

        start = max(start, self.time_bins_low[0])
        end = min(end, self.time_bins_high[-1])
        # make sure start time and end valid

        pixel_total_counts = np.sum(
            self.counts[(self.time_bins_low >= start) &
                        (self.time_bins_high <= end), :, :, emin_sci:emax_sci],
            axis=(0, 3))
        # timebin: detector,pixel, energies
    ...
    def plot_spectrogram(self, ax=None, selection_box=None):
        ...
        if selection_box:
            ...
            if erange and trange:
                ...
                tmin, tmax = sdt.utc2datetime(trange[0]), sdt.utc2datetime(
                    trange[1])
    ...
    def get_time_ranges_for_imaging(self,
                                    flare_time_ranges,
                                    imaging_energies,
                                    min_counts=3000,
                                    integration_time=60):
        ...
        for energy_range in imaging_energies:
            elow_sci, emax_sci = self.energy_to_index(energy_range[0],
                                                      energy_range[1])
        ...
        for i, sci_range in enumerate(sci_energy_ranges):
            for flare in flare_time_ranges:
                start, end, total_counts, pixel_counts = self.get_time_and_counts(
                    sci_range[0], sci_range[1], integration_time,
                    flare['peak_unix_time'] - integration_time / 2.,
                    flare['peak_unix_time'] + integration_time / 2)
