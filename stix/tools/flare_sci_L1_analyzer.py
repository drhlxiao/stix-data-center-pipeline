...
GRID_OPEN_AREA_RATIO = np.array([
    0.292312, 0.272487, 0.252282, 0.257189, 0.281502, 0.27254, 0.292103,
    0.260207, 0.335213, 0.305518, 0.335412, 0.254877, 0.264532, 0.255886,
    0.304781, 0.335199, 0.30505, 0.258, 0.257187, 0.252324, 0.281503, 0.260178,
    0.280854, 0.257033, 0.264449, 0.261743, 0.292299, 0.272491, 0.264514,
    0.254682
])
...
class SciL1Analyzer(object):
    ...
    def get_flare_spectra_and_location(self, flare_bsd_doc, flare_doc):
        ...
        if not self.time_integrated_counts:
            ...
            return None
        bkg_cursor = list( #look for bulk science data which contains background
            collection.find({
                '_id': {
                    '$lt': bsd_id
                },
                'request_form.emin':
                flare_bsd_doc['request_form']['emin'],
                'request_form.emax': {
                    '$gte': flare_bsd_doc['request_form']['emax']
                },
                'request_form.detector_mask':
                flare_bsd_doc['request_form']['detector_mask'],
                'request_form.eunit':
                flare_bsd_doc['request_form']['eunit'],
                'is_background':
                True,
                'SPID':
                54115
        ...
        if not bkg_cursor:
            print("Can not find background bsd data for bulk science data #",
                  bsd_id)
            return None
        bkg_doc = bkg_cursor[0]
        bkg_stat = bkg_doc['time_integrated_counts']
        sig_counts = np.array(
            self.time_integrated_counts['detector_total_counts'])
        ...
        bkg_cfl_rate_error = np.array(bkg_stat['pixel_rate_error'][8 * 12:9 *
                                                                   12, :])
        cfl_pixel_counts = np.array(
            self.time_integrated_counts['pixel_total_counts'][8 * 12:9 *
                                                              12, :])
        ...
        bkg_rate_error = bkg_rate_error[:, 0:len(
            sig_energy_map)]  #make the same size
        sig_bkgsub_counts = sig_counts - sig_duration * bkg_rates
        sig_bkgsub_count_error = np.sqrt(sig_counts + sig_duration *
                                         sig_duration * bkg_rate_error**2)

        results['detector_spectra_subtracted_bkg'] = sig_bkgsub_counts
        results[
            'detector_spectra_subtracted_bkg_error'] = sig_bkgsub_count_error
        
        # prepare background subtracted counts for CFL
        det_mean_counts = np.mean(
            np.sum(sig_bkgsub_counts[DET_ID_START:DET_ID_END, min_bin:max_bin],
                   axis=1) /
            GRID_OPEN_AREA_RATIO[DET_ID_END:DET_SENSITVE_AREA])
        det_mean_counts_error = np.sum(
            sig_bkgsub_count_error[DET_ID_START:DET_ID_END, min_bin:max_bin],
            axis=1)**2 / GRID_OPEN_AREA_RATIO[DET_ID_END:DET_SENSITVE_AREA]

        results[
            'cfl_pixel_spectra_subtracted_bkg_4_12_keV'] = cfl_pixel_bkgsub_counts[:,
                                                                                   min_bin:
                                                                                   max_bin]
        results[
            'cfl_pixel_spectra_subtracted_bkg_error_4_12_keV'] = cfl_pixel_bkgsub_count_error[:,
                                                                                              min_bin:
                                                                                              max_bin]
        detector_4_12_keV_counts = np.sum(sig_bkgsub_counts[:,
                                                            min_bin:max_bin],
                                          axis=1)

        return results

    def compute_detector_counts(self,
                                cursor,
                                min_unix_time=None,
                                max_unix_time=None):
        #calculate spectrogram
        #can be replaced with fits file in the future. 
        detector_counts = np.zeros(
            (32, 32))  #32 energy spectrum and 32 energy bins
        pixel_counts = np.zeros(
            (32 * 12, 32))  #32 energy spectrum and 32 energy bins
        energy_map = []
        for pkt in cursor:
        ...
        self.time_integrated_counts = {
            ...
            f'32 detectors x {num_energy_bin} spectra, rates (cnts/s)',
            ...
            'pixel_description':
            f'384 pixels x {num_energy_bin} spectra, rates (cnts/s)',
...
def process_L1_BSD_in_file(file_id):
    ...
    for doc in bsd_cursor:
        ...
        flares = list(
            mdb.search_flares_by_tw(bsd_start_unix, bsd_duration, threshold=0))
        packet_cursor = mdb.get_packets_of_bsd_request(db_id,
                                                       header_only=False)
        analyzer = SciL1Analyzer()
        result = analyzer.compute_detector_counts(packet_cursor,
                                                  bsd_start_unix, bsd_end_unix)
        ...
        if flares:
            ...
            flare_level1_result = analyzer.get_flare_spectra_and_location(
                bsd_doc, flare_doc)
