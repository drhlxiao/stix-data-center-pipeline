...
def merge_peaks(xpeaks, properties, unix_time, lc_smoothed):
    if xpeaks.size==0:
        return None
    intervals=[i for i in zip(properties['left_ips'], properties['right_ips'])]
    range_indices = merge_intervals(intervals)
    peaks={}
    peak_heights={}
    for ia, a in enumerate(range_indices):
        lc=lc_smoothed[a[0],a[1]]
        peak_counts=np.max(lc)
        wm=np.where(lc_smoothed == peak_counts)
        peak_times=unix_time[wm]

        peaks[ia]={'sub_peaks':[], 'peak_counts': np.max(lc_smoothed[a[0],a[1]),
            'peak_time':,
                'total_counts': 0,
                'total_signal_counts':0,
                }
        for ib, b in enumerate(intervals):
            if b[0]>=a[0] and b[1]<=a[1]:
                peaks[ia]['sub_peaks'].append(ib)
    ...
    total_counts = [
        int(np.sum(lightcurve[r[0]:r[1]])) for r in range_indices
    ]
...
def search(run_id,
    ...
    xpeaks, properties = signal.find_peaks(
        lc_smoothed,
        height=height,
        prominence=prominence, #2sigma above
    ...
    doc = {}
    range_indices = merge_peaks(xpeaks, properties, unix_time, lightcurve, lc_smoothed)
    ...
    seconds_per_bin=4
    durations=np.array([(r[1]-r[0])*seconds_per_bin for r in range_indices])
